function initRutaUI() {
    ...
    const uiHtml = `
    <div id="ruta-interface" style="position: absolute; bottom: 0; left: 0; width: 100%; background: linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0) 100%); padding: 12px 0; display: flex; align-items: center; justify-content: space-between; z-index: 500;">
    ...
    document.getElementById('ruta-final-btn').onclick = function() {
        if (checkAuth()) {
            ...
            if (typeof goToForm === 'function') {
                ...
                const masterBtn = Array.from(document.querySelectorAll('a, button')).find(el => el.textContent.includes('ЗАЯВКУ') && !el.id.includes('ruta'));
                if (masterBtn) masterBtn.click();
            }

            // ПЕРЕХОПЛЕННЯ: чекаємо моменту входу і перенаправляємо на register.html
            ...
            // Зупиняємо перевірку через 2 хвилини, якщо користувач передумав
